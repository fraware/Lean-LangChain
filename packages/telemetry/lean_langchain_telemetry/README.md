...
- **OTLP:** Set `OBR_OTLP_ENDPOINT` or `OTEL_EXPORTER_OTLP_ENDPOINT` (e.g. `http://localhost:4317`). Install optional deps: `pip install lean-langchain-telemetry[otlp]`. `OtlpTracer` converts each event to an OpenTelemetry span and exports via OTLP (gRPC).
- **LangSmith:** Set `LANGCHAIN_API_KEY` or `LANGCHAIN_TRACING_V2`. Install optional: `pip install lean-langchain-telemetry[langsmith]`. `LangSmithTracer` sends run-like data to LangSmith.
