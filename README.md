...
**Lean-LangChain** — a semantic control plane for high-stakes agent workflows (Obligation Runtime).

**Canonical naming:** The product is **Lean-LangChain**; PyPI root is **obligation-runtime**; npm TypeScript SDK is **`@lean-langchain/sdk`** per [docs/architecture/naming.md](docs/architecture/naming.md).
...
| Directory | Contents |
|-----------|----------|
| **Root** | [pyproject.toml](pyproject.toml): workspace metapackage `obligation_runtime` (enables `pip install -e ".[dev]"`), shared Ruff/Black/mypy/pytest config, optional extras (`dev`, `full`). |
