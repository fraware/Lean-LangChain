...
`packages/policy/lean_langchain_policy/engine.py` — Pure, deterministic evaluation over normalized evidence. Inputs: obligation, interactive_result, batch_result, patch_metadata, policy_pack. Output: `PolicyDecision` with `decision`, `trust_level`, `reasons`.
