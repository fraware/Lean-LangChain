...
class MCPSessionContext:
    ...
    def set_from_create_session(self, session_id: str, fingerprint_id: str, workspace_path: str = "") -> None:
...
def build_mcp_tools(
    ...
    def open_environment(repo_id: str, repo_path: str | None = None, repo_url: str | None = None, commit_sha: str = "HEAD") -> dict:
        return client.open_environment(repo_id=repo_id, repo_path=repo_path, repo_url=repo_url, commit_sha=commit_sha)

    def create_session(fingerprint_id: str, thread_id: str | None = None) -> dict:
        if thread_id:
            ctx.thread_id = thread_id
        out = client.create_session(fingerprint_id=fingerprint_id)
        session_id = out["session_id"]
        fingerprint_id = out.get("fingerprint_id", fingerprint_id)
        workspace_path = out.get("workspace_path", "")
        ctx.set_from_create_session(session_id=session_id, fingerprint_id=fingerprint_id, workspace_path=workspace_path)
        if store:
            store.set(
    ...
    def _session_id(session_id: str | None = None, thread_id: str | None = None) -> str:
        ...
        if not sid:
            raise ValueError("No session_id provided and no session in context. Call obligation/create_session first.")
        return sid

    def apply_patch(session_id: str | None = None, thread_id: str | None = None, files: dict[str, str] | None = None) -> dict:
        return client.apply_patch(session_id=_session_id(session_id, thread_id), files=files or {})

    def check_interactive(session_id: str | None = None, thread_id: str | None = None, file_path: str = "") -> dict:
        return client.interactive_check(session_id=_session_id(session_id, thread_id), file_path=file_path)

    def get_goal(session_id: str | None = None, thread_id: str | None = None, file_path: str = "", line: int = 0, column: int = 0, goal_kind: str = "plainGoal") -> dict:
        return client.get_goal(session_id=_session_id(session_id, thread_id), file_path=file_path, line=line, column=column, goal_kind=goal_kind)

    def batch_verify(session_id: str | None = None, thread_id: str | None = None, target_files: list[str] | None = None, target_declarations: list[str] | None = None) -> dict:
        return client.batch_verify(session_id=_session_id(session_id, thread_id), target_files=target_files or [], target_declarations=target_declarations or [])

    def get_review_payload(thread_id: str) -> dict:
