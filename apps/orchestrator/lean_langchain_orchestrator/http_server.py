...
Run with: uvicorn lean_langchain_orchestrator.http_server:app --host 0.0.0.0 --port 8001
"""
...
from lean_langchain_schemas.api_paths import PREFIX, PATH_REVIEW_RESUME
from lean_langchain_schemas.orchestrator_api import (
    OrchestratorCapabilityBlock,
    OrchestratorHealthResponse,
    OrchestratorReadyResponse,
)

from lean_langchain_orchestrator.orchestrator_capabilities import (
...
async def _lifespan(app: FastAPI):
    log = logging.getLogger("lean_langchain_orchestrator")
...
def create_app() -> FastAPI:
    ...
    def resume(thread_id: str, body: ResumeBody) -> dict:
        ...
        try:
            from lean_langchain_orchestrator.runtime.graph import (
                build_patch_admissibility_graph,
            )
            from lean_langchain_orchestrator.runtime.initial_state import make_resume_state
