...
def create_node_handlers(
    ...
    def finalize(state: ObligationRuntimeState) -> dict[str, Any]:
        from lean_langchain_schemas.common import new_id
        ...
        from lean_langchain_schemas.witness import WitnessBundle

        env = state.get("environment_fingerprint") or {}
        inter = state.get("interactive_result") or {}
        batch_raw = state.get("batch_result")
        from lean_langchain_schemas.witness import AcceptanceSummary
